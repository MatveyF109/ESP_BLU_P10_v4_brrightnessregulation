...
void QTRSensors::setTypeAnalogMX() {
  _type = QTRType::AnalogMX;
...
void QTRSensors::setSensorPins(const uint8_t *pins, uint8_t sensorCount) {
  ...
  if (_type == QTRType::AnalogMX) {
    _ADC_PIN = _sensorPins[0];
    _S0 = _sensorPins[1];
    _S1 = _sensorPins[2];
    _S2 = _sensorPins[3];
    _S3 = _sensorPins[4];
...
void QTRSensors::emittersOff(QTREmitters emitters, bool wait) {
  ...
  if (emitters == QTREmitters::All || (_emitterPinCount == 2 && emitters == QTREmitters::Odd)) {
    // Check if pin is defined and only turn off if not already off
    if ((_oddEmitterPin != QTRNoEmitterPin) && (digitalRead(_oddEmitterPin) == HIGH)) {
  ...
  if (_emitterPinCount == 2 && (emitters == QTREmitters::All || emitters == QTREmitters::Even)) {
    // Check if pin is defined and only turn off if not already off
    if ((_evenEmitterPin != QTRNoEmitterPin) && (digitalRead(_evenEmitterPin) == HIGH)) {
...
void QTRSensors::emittersOn(QTREmitters emitters, bool wait) {
  ...
  if (emitters == QTREmitters::All || (_emitterPinCount == 2 && emitters == QTREmitters::Odd)) {
    ...
    // care of this)
    if ((_oddEmitterPin != QTRNoEmitterPin) && (_dimmable || (digitalRead(_oddEmitterPin) == LOW))) {
  ...
  if (_emitterPinCount == 2 && (emitters == QTREmitters::All || emitters == QTREmitters::Even)) {
    ...
    // we might be changing the dimming level (emittersOnWithPin() should take
    // care of this)
    if ((_evenEmitterPin != QTRNoEmitterPin) && (_dimmable || (digitalRead(_evenEmitterPin) == LOW))) {
...
void QTRSensors::calibrate(QTRReadMode mode) {
  ...
  } else if (mode == QTRReadMode::OddEven || mode == QTRReadMode::OddEvenAndOff) {
    calibrateOnOrOff(calibrationOn, QTRReadMode::OddEven);
  }

  if (mode == QTRReadMode::OnAndOff || mode == QTRReadMode::OddEvenAndOff || mode == QTRReadMode::Off) {
...
void QTRSensors::readCalibrated(uint16_t *sensorValues, QTRReadMode mode) {
  ...
  if (mode == QTRReadMode::On || mode == QTRReadMode::OnAndOff || mode == QTRReadMode::OddEvenAndOff) {
    if (!calibrationOn.initialized) {
      return;
    }
  }

  if (mode == QTRReadMode::Off || mode == QTRReadMode::OnAndOff || mode == QTRReadMode::OddEvenAndOff) {
...
void QTRSensors::readPrivate(uint16_t *sensorValues, uint8_t start, uint8_t step) {
  ...
  switch (_type) {
    ...
    case QTRType::AnalogMX:
      ...
      for (uint8_t j = 0; j < _samplesPerSensor; j++) {
        for (uint8_t i = start; i < _sensorCount; i += step) {
          setMXChannel(i + _channelStartNo);
          // add the conversion result
          int value = 0;
          adc2_get_raw(gpio2ADCchannel(_ADC_PIN), ADC_WIDTH_BIT_10, &value);
          //Serial.println(value);
          value = (1024 - value);
          sensorValues[i] += value;
          //sensorValues[i] += analogRead(_channel);
        }
      }
...
uint16_t QTRSensors::readLinePrivate(uint16_t *sensorValues, QTRReadMode mode, bool invertReadings) {
  if (!_started && _startTime == 0)
    _startTime = millis();
  else if (!_started)
    if (millis() - _startTime > 100) _started = true;
  ...
  _lastPosition1 = _lastPosition;
  uint16_t _currPosition = avg / sum;


  //check if we are crossing hor line
  if (_crossingWidth > 0)
    _onCrossing = getSensorOnLineAjacentCount(sensorValues) >= _crossingWidth;

  switch (_smoothing) {
    case QTRSmoothing::None:
      _lastPosition = _currPosition;
      break;

    case QTRSmoothing::Double:
      _lastPosition = (_currPosition + _lastPosition1) / 2;
      break;

    case QTRSmoothing::Triple:
      _lastPosition = (_currPosition + _lastPosition1 + _lastPosition2) / 3;
      break;

    case QTRSmoothing::Filter:
      {
        if (_started)
          _lastPosition = (abs(_currPosition - _lastPosition1) > _filterLevel) ? _lastPosition1 : _currPosition;
        else
          _lastPosition = _currPosition;
        break;
      }
...
void QTRSensors::setBlackLevel(int blackLevel) {
  _blackLevel = blackLevel;
}

void QTRSensors::setCrossingWidth(const uint8_t crossingWidth) {
  _crossingWidth = crossingWidth;
}

void QTRSensors::setChannelStartNo(const uint8_t channelStartNo) {
  _channelStartNo = channelStartNo;
}

void QTRSensors::setSmoothing(QTRSmoothing smoothing) {
  _smoothing = smoothing;
}

// void QTRSensors::setSmoothingDouble() {
//   _smoothing = QTRSmoothing::Double;
// }

// void QTRSensors::setSmoothingTriple() {
//   _smoothing = QTRSmoothing::Triple;
// }

uint16_t QTRSensors::getSensorBinary(uint16_t *sensorValues) {
  uint16_t binaryValue = 0, bitmask = 0b0000000000000001;
...
uint16_t QTRSensors::getSensorOnLineCount(uint16_t *sensorValues) {
  int blackCnt = 0;
  for (uint8_t i = 0; i < _sensorCount; i++) {
    blackCnt += sensorValues[i] > _blackLevel ? 1 : 0;
  }
  return blackCnt;
}

uint16_t QTRSensors::getSensorOnLineAjacentCount(uint16_t *sensorValues) {
  int blackAjacentCnt = 0;
  int i = 0;
  int c = 0;
  bool packetCounting = false;
  uint16_t sensorCounts[QTRMaxSensors];
  memset(sensorCounts, 0, sizeof(sensorCounts));
  while (i < _sensorCount) {
    if (!packetCounting && sensorValues[i] > _blackLevel) {
      packetCounting = true;
      while (packetCounting) {
        if (sensorValues[i] <= _blackLevel) {
          packetCounting = false;
          sensorCounts[c] = blackAjacentCnt;
          blackAjacentCnt = 0;
          c += 1;
        } else {
          blackAjacentCnt += 1;
  ...
  int maxAjacentCnt = 0;
  for (int i = 0; i < QTRMaxSensors; i++) {
    if (sensorCounts[i] > maxAjacentCnt)
      maxAjacentCnt = sensorCounts[i];
  }

  return maxAjacentCnt;
}



adc2_channel_t QTRSensors::gpio2ADCchannel(int gpio_pin) {
  switch (gpio_pin) {
    case GPIO_NUM_0: return ADC2_CHANNEL_1;
    case GPIO_NUM_2: return ADC2_CHANNEL_2;
    case GPIO_NUM_4: return ADC2_CHANNEL_0;
    ...
    case GPIO_NUM_25: return ADC2_CHANNEL_8;
...
void QTRSensors::setMXChannel(int mxChannel) {
  digitalWrite(_S0, _g_channel_truth_table[mxChannel][0]);
  digitalWrite(_S1, _g_channel_truth_table[mxChannel][1]);
  digitalWrite(_S2, _g_channel_truth_table[mxChannel][2]);
  digitalWrite(_S3, _g_channel_truth_table[mxChannel][3]);
}

void printSensorValues(String debugNo, uint16_t sensorCount, uint16_t *sensorValues) {
  if (!PRINT_DEBUG) return;

  Serial.print(debugNo + ">");
  for (uint8_t i = 0; i < sensorCount; i += 1) {
