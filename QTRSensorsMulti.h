...
/// Smoothing type.
enum class QTRSmoothing : uint8_t {
  None,
  Double,
  Triple,
  Filter
...
public:
  ...
  /// \brief Specifies that the sensor type is analogMX.
  /// Means the sensor works via CD74HC4067 multiplexer
  /// Call this function to set up MX-type sensors.
  void setTypeAnalogMX();

  //todo: remove
  void setCrossingWidth(const uint8_t crossingWidth);

  void setChannelStartNo(const uint8_t channelStartNo);

  void setSmoothing(QTRSmoothing smoothing);

  void setBlackLevel(int blackLevel);

  // void setSmoothingTriple();

  uint16_t getSensorBinary(uint16_t *sensorValues);

  uint16_t getSensorOnLineCount(uint16_t *sensorValues);

  uint16_t getSensorOnLineAjacentCount(uint16_t *sensorValues);
...
private:
  ...
  //new fields
  QTRSmoothing _smoothing = QTRSmoothing::None;

  uint8_t _crossingWidth = 0;

  uint8_t _channelStartNo = 0;

  bool _onCrossing = false;

  int _blackLevel = 200;

  int _filterLevel = 5000;

  uint16_t _lastPosition = 0;

  uint16_t _lastPosition1 = 0;

  uint16_t _lastPosition2 = 0;
  ...
  //CD74HC4067 support
  //taken from standard Arduino IDE lib
  int _S0;
  int _S1;
  int _S2;
  int _S3;
  ...
  int _g_channel_truth_table[15][4] = {
    // s0, s1, s2, s3     channel
    // { 1, 1, 1, 0 },  // 0 (I7 - OUT1)
    // { 0, 1, 1, 0 },  // 1 (I6 - OUT2)
    // { 1, 0, 1, 0 },  // 2 (I5 - OUT3)
    ...
    // { 0, 1, 1, 1 },  // 8 (I14- OUT9)
    // { 1, 0, 1, 1 },  // 9 (I13 - OUT10)
    // { 0, 0, 1, 1 },  // 10 (I12 - OUT11)
    // { 1, 1, 0, 1 },  // 11 (I11 - OUT12)
    // { 1, 0, 1, 0 },  // 12 (I10 - OUT13)
    // { 1, 0, 0, 1 },  // 13 (I9 - OUT14)
    // { 0, 0, 0, 1 },  // 14 (I8 - OUT15)
    // { 1, 1, 1, 1 },  // 15 (I15 - NC)
